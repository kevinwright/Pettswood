...
class DomainBridgeSpec extends Specification with Mockito {
  ...
  "A DomainBridge" should {
    ...
    "count the results and provide a summary of them" in {
      ...
      domain.summary must be equalTo ResultSummary(domain, List(PASS, FAIL, FAIL, SETUP, EXCEPTION) reverse)
    }
    ...
    "gather results from its nestlings" in {
      ...
      domain.summary must be equalTo ResultSummary(domain,List(PASS, PASS, FAIL, SETUP, EXCEPTION))
    }
  }
}
